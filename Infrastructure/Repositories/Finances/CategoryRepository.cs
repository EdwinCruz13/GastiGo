...
namespace Infrastructure.Repositories.Finances
{
    public class CategoryRepository : ICategoryRepository
    {
        ...
        {
            return await _context.Categories.Where(x => x.UserId == userId).Include(n => n.Nature).Include(p => p.Params).OrderByDescending(x => x.Nature.Name).ThenBy(x => x.CreatedAt).ToListAsync();
        }
